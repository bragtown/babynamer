...
angular.module('babynamerApp')
  .controller('BoyCtrl', function ($scope, $firebase) {
    document.body.style.background = 'lightblue';
    var firstRef = new Firebase('https://babynamer.firebaseIO.com/boys/first');
    $scope.firsts = $firebase(firstRef);
    var middleRef = new Firebase('https://babynamer.firebaseIO.com/boys/middle');
    $scope.middles = $firebase(middleRef);
    var lastRef = new Firebase('https://babynamer.firebaseIO.com/boys/last');
    $scope.lasts = $firebase(lastRef);
    var likeRef = new Firebase('https://babynamer.firebaseIO.com/boys/like');
    ...
    $scope.generateName = function(){
    	...
    	var gnameRef = new Firebase('https://babynamer.firebaseIO.com/boys/first/'+firstgname);
      ...
      gnameRef = new Firebase('https://babynamer.firebaseIO.com/boys/middle/'+middlegname);
      gnameRef.on('value', function(snapshot) {
        $scope.name.middle = snapshot.val().body;
      });
      names = $scope.lasts.$getIndex();
      var lastgname = names[Math.round(Math.random() * (names.length - 1))];
      gnameRef = new Firebase('https://babynamer.firebaseIO.com/boys/last/'+lastgname);
      gnameRef.on('value', function(snapshot) {
